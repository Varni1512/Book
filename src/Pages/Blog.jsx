        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {blogs.map(blog => (
            <div key={blog.id} className="bg-white rounded-[24px] overflow-hidden shadow-[0_4px_25px_rgba(0,0,0,0.04)] border border-gray-100 flex flex-col group hover:shadow-[0_8px_30px_rgba(0,0,0,0.08)] transition-shadow duration-300">
              <Link to={`/blog/${blog.slug || generateSlug(blog.title)}`} className="block relative w-full aspect-[4/3] overflow-hidden bg-gray-100">
                <img src={blog.image} alt={blog.imageAlt || blog.title} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500" />
                {blog.category && (
                  <div className="absolute top-4 left-4 bg-white/95 backdrop-blur-sm px-3.5 py-1.5 rounded-full text-[10px] font-bold text-[#5588CB] uppercase tracking-wider shadow-sm">
                    {blog.category}
                  </div>
                )}
              </Link>
              ...
              <div className="p-4 flex flex-col flex-grow">
                ...
                <Link to={`/blog/${blog.slug || generateSlug(blog.title)}`}>
                ...
                <div className="mt-auto pt-5 border-t border-gray-100">
                  <Link to={`/blog/${blog.slug || generateSlug(blog.title)}`} className="text-[#5588CB] font-['Inter',_sans-serif] font-bold text-[11px] tracking-wider uppercase flex items-center gap-2 hover:text-[#4875b3] transition-colors">
                    Read Article
